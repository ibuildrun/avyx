...
export interface Task {
  ...
  minLevel?: number;
  isPremium?: boolean;
  isEscrowProtected?: boolean; // Защищена ли сделка
  challenge?: Challenge; // Специальный вызов для задачи
}
...
export interface Sprint {
  ...
  heroOfTheDay?: string; // userId of the winner
}
...
export interface User {
  ...
  streak: number;
  rank: number;
  totalEarnings: number; // Общий заработок для "Сада"
  about?: string;
  bannerImage?: string;
  ...
  maxProposals: number;
  hasShield: boolean;
  badges: Badge[]; // Список достижений
  appliedTasks?: string[]; // Список ID задач, на которые уже подан отклик
  isHeroOfDay?: boolean; // Статус "Герой дня"
  mood?: Mood; // Настроение дня
}
