...
type Sorting = 'popular' | 'new';

const Icons = {
  Lock: () => (
    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><rect width="18" height="11" x="3" y="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
  ),
  Zap: () => (
    <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor"><path d="M13 2 3 14h9l-1 8 10-12h-9l1-8z"/></svg>
  ),
  Trophy: () => (
...
const HomeScreen: React.FC<HomeScreenProps> = ({ user, onTaskSelect, onWorkSelect, onOpenNotifications }) => {
  ...
  // Enhance tasks with challenges
  const enhancedTasks: Task[] = MOCK_TASKS.map((t, idx) => {
    if (idx === 0) return { ...t, isFeatured: true, isPremium: true, challenge: { type: 'speed', label: 'Speed Demon', reward: '+100 Stars' } };
    if (idx === 1) return { ...t, minLevel: 3, challenge: { type: 'bonus', label: 'Double XP', reward: 'x2 Experience' } };
    return t;
  });
  ...
  return (
    <div className="p-4 space-y-6 pb-20">
      ...
      {/* Recommended Designers for Clients / Community Focus */}
      {activeTab === 'tasks' && (
        ...
        <section className="space-y-6 animate-in fade-in duration-300">
          ...
          <div className="space-y-6">
            {filteredTasks.map((task) => {
              const isLocked = task.minLevel && user.level < task.minLevel;
              const isPremiumLocked = task.isPremium && !user.isPro;
